...
class KeywordsViewlet(ViewletBase):
    ...
    def update(self):
        ...
        if anonymous:
            try:
                self.show_tags = aq_acquire(self.context, 'show_tags')
            except AttributeError:
...
class DocumentBylineViewlet(DocumentBylineViewlet):
    ...
    def show(self):
        ...
        #ability to locally override showing the author.

        show_author = True

        if anonymous:
            try:
                show_author = aq_acquire(self.context, 'show_author')
            except AttributeError:
                show_author = False

        return not anonymous or allowAnonymousViewAbout or show_author

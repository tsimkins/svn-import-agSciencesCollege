Description
    ...
    agSciPolicy is a product that adds a new style to a Plone 3.0.x portal.
    It adds a new skin selection to the 'portal_skins' tool
    (called agSciPolicy), and registers a custom stylesheet (called 
    main.css) with the 'portal_css' tool.

    agSciPolicy is based on DIYPloneStyle 3.0, a skeleton product
    ready for building new graphical designs for Plone.

Installation

    On the file system: place agSciPolicy in the Products directory
    of your Zope instance and restart the server.

    In the Plone Web Interface: as portal manager, go to 'Portal > Site Setup
    > Add-on Products'.
    Select 'agSciPolicy' and click the *Install* button.
...
Selecting a skin
    ...
    profiles/default/skins.xml), the agSciPolicy skin will be selected (or
